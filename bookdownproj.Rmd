...
knitr::include_graphics('images/EHRVAL.jpg', dpi = NA)
```
...
<!--chapter:end:02-Simple_Differential_Equations.Rmd-->
...
<!--chapter:end:03-Nonlinear_Differential_Equations.Rmd-->
...
<!--chapter:end:04b-PhaseLine_examples.Rmd-->
...
<!--chapter:end:06-Example_Sys.Rmd-->
...
<!--chapter:end:07b-Balance_Example.Rmd-->
...
<!--chapter:end:08-Spiral.Rmd-->

# Memory Matters: Introduction to Delay Differential Equations
...
## The Mathematical Structure of Delay Systems
...
## Stability Analysis and Characteristic Equations
...
\[i\omega = -be^{-i\omega\tau} = -b(\cos(-\omega\tau) + i\sin(-\omega\tau))\]
\[i\omega = -b\cos(\omega\tau) + ib\sin(\omega\tau)\]

Equating real and imaginary parts:
1. \(0 = -b\cos(\omega\tau) \implies \omega\tau = \pi/2\) (since \(\cos(\theta)=0\) at \(\pi/2\))
2. \(\omega = b\sin(\omega\tau) \implies \omega = b\) (since \(\sin(\pi/2)=1\))
...
# Scalar linear delay equation: dx/dt = -ax(t) - bx(t-tau)
...
## Population Dynamics with Maturation Delays
...
## Medical Applications: Infectious Disease with Latency
...
\[\frac{dI}{dt} = \beta S(t-\tau)I(t-\tau)e^{-\mu\tau} - \gamma I(t)\]
...
## Drug Pharmacokinetics and Treatment Delays
...
\[\frac{dA}{dt} = - k_a A(t)\]
\[\frac{dC}{dt} = \frac{k_a A(t-\tau)}{V} - k_e C(t)\]
...
By using event-based dosing, we ensure that every milligram of the drug is accounted for, regardless of the solver's step size. The delay \(\tau\) shifts the peak concentration relative to the dosing time, a critical factor in determining whether a drug stays within its therapeutic window.

## Immunological Memory and Delayed Response
...
\[\frac{dV}{dt} = rV(t)\left(1 - \frac{V(t)}{K}\right) - pV(t)E(t)\]
\[\frac{dE}{dt} = cV(t-\tau) - \delta E(t)\]
...
<!--chapter:end:10-past_present.Rmd-->
...
<!--chapter:end:11-TippingPoints.Rmd-->
