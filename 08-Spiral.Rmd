...
# FitzHugh-Nagumo cardiac oscillator model
...
vector_field$dV <- with(vector_field, 
  fn_params$c * (V - V^3/3 + R))
vector_field$dR <- with(vector_field, 
  -(V - fn_params$a + fn_params$b*R) / fn_params$c)

# Normalize vectors
vector_field$magnitude <- sqrt(vector_field$dV^2 + vector_field$dR^2)
vector_field$dV_norm <- vector_field$dV / (vector_field$magnitude + 0.1) * 0.12
vector_field$dR_norm <- vector_field$dR / (vector_field$magnitude + 0.1) * 0.12
...
phase_portrait <- ggplot() +
  geom_segment(data = vector_field[seq(1, nrow(vector_field), 3),],
               aes(x = V, y = R, xend = V + dV_norm, yend = R + dR_norm),
               arrow = arrow(length = unit(0.015, "npc")),
               color = "lightblue", alpha = 0.5) +
  geom_path(data = trajectory_df, aes(x = V, y = R),
            color = "darkred", size = 1.2, alpha = 0.8) +
  geom_point(data = trajectory_df[1,], aes(x = V, y = R),
             color = "green", size = 3) +
  geom_point(data = tail(trajectory_df, 1), aes(x = V, y = R),
             color = "blue", size = 3) +
  labs(title = "Cardiac Oscillator: FitzHugh-Nagumo Limit Cycle",
       x = "Membrane Potential (V)",
       y = "Recovery Variable (R)",
       subtitle = "Trajectory spirals toward stable periodic orbit") +
  theme_minimal() +
  coord_equal(xlim = c(-2.5, 2.5), ylim = c(-2, 2))

# Time series showing ECG-like pattern
timeseries_plot <- ggplot(trajectory_df, aes(x = time)) +
  geom_line(aes(y = V), color = "darkred", size = 1) +
  labs(title = "Simulated ECG Pattern",
       x = "Time (arbitrary units)",
       y = "Voltage (V)",
       subtitle = "Self-sustained oscillations mimic cardiac rhythm") +
  theme_minimal() +
...
```{r}
# Demonstrate Hopf bifurcation by varying parameter
...
for (i in seq_along(a_values)) {
  ...
  # Integrate for long time to reach attractor
  times_long <- seq(0, 50, by = 0.05)
  traj_temp <- ode(y = c(V = 0.5, R = 0), times = times_long,
                   func = fitzhugh_nagumo, parms = params_temp)
  
  # Extract last period to measure amplitude
  last_segment <- tail(traj_temp, 200)
...
bifurcation_plot <- ggplot(bifurcation_diagram, aes(x = a, y = amplitude)) +
  ...
  annotate("text", x = 0.5, y = 3, label = "Stable\nFixed Point", size = 4) +
  annotate("text", x = 0.85, y = 3, label = "Limit Cycle\nOscillations", size = 4) +
  labs(title = "Hopf Bifurcation in Cardiac Model",
...
fitzhugh_refractory <- function(t, state, parameters) {
  with(as.list(c(state, parameters)), {
    # Add refractory blocking current
...
normal_params <- c(fn_params, list(block_strength = 0.3))
disease_params <- c(fn_params, list(block_strength = 0.05))

# Compare trajectories
times_compare <- seq(0, 40, by = 0.01)

traj_normal <- ode(y = c(V = 0.5, R = 0), times = times_compare,
                   func = fitzhugh_refractory, parms = normal_params)
traj_disease <- ode(y = c(V = 0.5, R = 0), times = times_compare,
                    func = fitzhugh_refractory, parms = disease_params)
...
compare_plot <- ggplot() +
  geom_line(data = traj_normal_df, aes(x = time, y = V, color = "Normal"),
            size = 0.8, alpha = 0.7) +
  geom_line(data = traj_disease_df, aes(x = time, y = V, color = "Disease"),
            size = 0.8, alpha = 0.7) +
  scale_color_manual(values = c("Normal" = "darkgreen", "Disease" = "darkred")) +
  labs(title = "Cardiac Dynamics: Normal vs. Disease State",
       x = "Time",
       y = "Voltage (V)",
       color = "State",
